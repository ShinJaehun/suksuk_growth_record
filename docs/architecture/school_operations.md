...
manager의 canonical source는 기존 `SchoolMembership.role == "manager"`이며 `School.manager_id`를 추가하지 않는다. 같은 school의 manager membership은 최대 하나이고 global admin은 이 수에 포함하지 않는다.
...
역할은 integer enum으로 구현되어 있으며 기본값은 `member`다. 교사 한 명은 최대 한 학교에만 소속되고 담당 학급은 없거나 하나다. 다른 학교 또는 다른 학년 teacher 배정은 차단하며 여러 학교 소속 지원은 현재 범위가 아니다. 한 학교의 manager는 없거나 한 명이고 global admin과 student는 SchoolMembership을 갖지 않는다.

학급 담당 교사는 학급과 같은 학교 및 같은 학년의 SchoolMembership을 가진 active teacher만 가능하다. `Classroom.teacher_id`는 한 teacher에게 최대 하나의 classroom만 연결하도록 DB uniqueness를 적용한다. 학교 manager의 운영 화면은 자기 학교의 단일 담당 관계만 변경하고 학교 소속과 manager 역할은 변경하지 않는다.
