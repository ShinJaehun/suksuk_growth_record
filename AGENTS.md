...
- 현재 runtime에서는 admin, teacher와 student가 `User`에 함께 존재한다.
- 승인된 target에서는 `User`가 admin/teacher만 담당하고, Classroom에 직접 속한 별도 `Student`는 Devise User가 아닌 Rails session + PIN으로 인증한다. 이는 일반적인 인증 모델 확대가 아니라 승인된 Student cutover 예외이며 그 밖의 role을 성급히 별도 인증 모델로 분리하지 않는다.
...
- 현재 runtime의 학생 소속은 student `ClassroomMembership`을 기준으로 한다. 최종 target은 특정 Classroom에 직접 속하는 별도 `Student`이며 `docs/specs/student_model_migration.md`를 따른다.
- Teacher, Student membership, Classroom lifecycle은 각각의 canonical spec과 상태 source를 따른다.
- 현재 runtime과 승인된 migration target을 구분한다. 학생 target에는 `StudentEnrollment`를 두지 않는다.
